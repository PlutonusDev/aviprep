...
export function parseSchedule3(raw: string): ParsedSchedule {
  ...
  /**
   * Tokens already spent in this element. Kept apart from the items, because a
   * paragraph demoted to a lead-in has still used its letter: ANVC 2.9.6 lists
   * "(a) (b) (a)", and that second "(a)" must not land back on "(b)".
   */
  const usedLetters = new Set<string>()
  let usedRomans = new Set<string>()
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (paraMatch && element && !wrappedStem) {
      ...
      /**
       * A repeated token takes the next one in sequence instead, so refs stay
       * unique and in reading order.
       */
      const nextFree = (wanted: string, expected: string, used: Set<string>, sequence: string[]) => {
        if (!used.has(wanted)) return wanted
        const from = Math.max(0, sequence.indexOf(expected))
        return sequence.slice(from).find((t) => !used.has(t)) ?? wanted
      }
      ...
      if (kind === "letter") {
        ...
        token = nextFree(token, expectedLetter, usedLetters, letters)
        usedLetters.add(token)
        usedRomans = new Set()
        ...
        if (letterItem && letterToken) {
          ...
          const chosen = nextFree(token, expectedRoman, usedRomans, ROMANS)
          usedRomans.add(chosen)
          const child: ParsedItem = { ref: `${current.number}(${letterToken})(${chosen})`, text: tidy(text) }
          ...
          const chosen = nextFree(token, expectedRoman, usedRomans, ROMANS)
          usedRomans.add(chosen)
          usedLetters.add(chosen)
          const item: ParsedItem = { ref: `${current.number}(${chosen})`, text: tidy(text) }
          current.items.push(item)
          target = item
        }
        lastRoman = token
